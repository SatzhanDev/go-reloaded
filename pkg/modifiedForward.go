...
func ChangeForward(changedArticle []string) string {
	var removedArr []string
	for i, v := range changedArticle {
		...
		if v == "(cap)" || v == "(cap)," || v == "(cap)." || v == "(cap):" || v == "(cap);" || v == "(cap)!" || v == "(cap)?" || v == "(cap)..." || v == "(cap)!?" {
			...
			binInt, _ := strconv.ParseInt(changedArticle[i-1], 2, 64)
			if v[len(v)-1] != ')' {
			...
			if changedArticle[i-1] == "(cap," || changedArticle[i-1] == "(up," || changedArticle[i-1] == "(low," {
				if v[len(v)-1] != ')' {
					removedArr[len(removedArr)-1] = removedArr[len(removedArr)-1] + string(v[len(v)-1])
				}
			} else {
				removedArr = append(removedArr, changedArticle[i])
			}
		} else if i == 0 {
			removedArr = append(removedArr, changedArticle[i])
		}
